...
public class ListarContato extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            GenericDao contatoDao = new ContatoDao();
            request.setAttribute("contatos", contatoDao.listar());
            request.getRequestDispatcher("listarContato.jsp").forward(request, response);
        } catch (Exception e) {
            System.out.println("Erro ao listar contatoCTR " + e.getMessage());
        }
    }
